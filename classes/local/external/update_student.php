...
/**
 * SELMA plugin 'update_student' external file.
 *
 * @package    enrol_selma
 * @category   external
 ...
 */
class update_student extends external_api {
    ...
    public static function update_student($student) {
        ...
        $params = self::validate_parameters(self::update_student_parameters(), ['student' => $student]);
        $status = 1;
        $moodleuserid = 0;
        $warnings = [];
        $selmadata = $params['student'];
        try {
            $user = enrol_selma_update_student_from_selma($selmadata, get_config('enrol_selma'));
            if ($user->id >= 1) {
                $status = 200;
